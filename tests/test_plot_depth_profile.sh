...
# plot_depth_profile.py -d vs -c ${MODEL} -o ${MODEL}_d_vs.png -i $UCVM_INSTALL_PATH -C "vertical profile vs ${MODEL}" -s ${LAT1},${LON1} -b ${START} -e ${END} -v ${STEP}
...
./ucvm_vertical_profile2csv.py ${MODEL}_d_d_vs_matprops.json ${MODEL}_d_d_vs_meta.json
